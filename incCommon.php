	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'equipos' => array('Equipos', '', 'resources/table_icons/autos.png', 'Anagrafica'),
			'modelos' => array('Modelos', '', 'resources/table_icons/chart_organisation.png', 'Anagrafica'),
			'familias' => array('Familias', '', 'resources/table_icons/teddy_bear.png', 'Anagrafica'),
			'repuestos' => array('Repuestos', '', 'resources/table_icons/cog.png', 'Anagrafica'),
			'articulos' => array('Articulos', '', 'resources/table_icons/basket_add.png', 'Anagrafica'),
			'marcas' => array('Marcas', '', 'resources/table_icons/factory.png', 'Anagrafica'),
			'equipo_repuestos' => array('Equipo repuestos', '', 'table.gif', 'hiddens'),
			'tecnicos' => array('Tecnicos', '', 'resources/table_icons/role.png', 'Anagrafica'),
			'planificaciones' => array('Planificaciones', '', 'resources/table_icons/application_from_storage.png', 'Planificacion'),
			'planificacion_equipos' => array('Planificacion equipos', '', 'table.gif', 'hiddens'),
			'codigo_servicios' => array('Codigo servicios', '', 'table.gif', 'hiddens'),
			'camionetas' => array('Camionetas', '', 'resources/table_icons/yellow_submarine.png', 'Anagrafica'),
			'clientes' => array('Clientes', '', 'resources/table_icons/account_balances.png', 'Anagrafica'),
			'planificacion_pendientes' => array('Planificacion pendientes', '', 'table.gif', 'Planificacion')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('Planificacion', 'Anagrafica', 'hiddens');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'planificacion_equipos' => "`planificacion_equipos`.`id` as 'id', IF(    CHAR_LENGTH(`planificaciones1`.`titulo`), CONCAT_WS('',   `planificaciones1`.`titulo`), '') as 'planificacion', IF(    CHAR_LENGTH(`clientes1`.`nombre`), CONCAT_WS('',   `clientes1`.`nombre`), '') as 'cliente', IF(    CHAR_LENGTH(`equipos1`.`interno`), CONCAT_WS('',   `equipos1`.`interno`), '') as 'equipo', IF(    CHAR_LENGTH(`modelos1`.`modelo`), CONCAT_WS('',   `modelos1`.`modelo`), '') as 'modelo', `planificacion_equipos`.`ubicacion` as 'ubicacion', `planificacion_equipos`.`horometro` as 'horometro', `planificacion_equipos`.`cumplido` as 'cumplido', `planificacion_equipos`.`comentarios` as 'comentarios', if(`planificacion_equipos`.`fecha_cumplido`,date_format(`planificacion_equipos`.`fecha_cumplido`,'%m/%d/%Y'),'') as 'fecha_cumplido', `planificacion_equipos`.`distancia` as 'distancia'",
			'codigo_servicios' => "`codigo_servicios`.`id` as 'id', `codigo_servicios`.`servicio` as 'servicio'",
			'camionetas' => "`camionetas`.`id` as 'id', `camionetas`.`interno` as 'interno', `camionetas`.`matricula` as 'matricula'",
			'clientes' => "`clientes`.`id` as 'id', `clientes`.`nombre` as 'nombre'",
			'planificacion_pendientes' => "`planificacion_pendientes`.`id` as 'id', IF(    CHAR_LENGTH(`planificaciones1`.`titulo`), CONCAT_WS('',   `planificaciones1`.`titulo`), '') as 'planificacion', IF(    CHAR_LENGTH(`equipos1`.`interno`), CONCAT_WS('',   `equipos1`.`interno`), '') as 'interno', `planificacion_pendientes`.`comentario` as 'comentario'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(   
			'equipos' => "`equipos` LEFT JOIN `articulos` as articulos1 ON `articulos1`.`id`=`equipos`.`articulo` LEFT JOIN `marcas` as marcas1 ON `marcas1`.`id`=`equipos`.`marca` LEFT JOIN `modelos` as modelos1 ON `modelos1`.`id`=`equipos`.`modelo` LEFT JOIN `familias` as familias1 ON `familias1`.`id`=`equipos`.`familia` ",
			'modelos' => "`modelos` LEFT JOIN `marcas` as marcas1 ON `marcas1`.`id`=`modelos`.`marca` ",
			'familias' => "`familias` ",
			'repuestos' => "`repuestos` ",
			'articulos' => "`articulos` ",
			'marcas' => "`marcas` ",
			...
			'clientes' => "`clientes` ",
			'planificacion_pendientes' => "`planificacion_pendientes` LEFT JOIN `planificacion_equipos` as planificacion_equipos1 ON `planificacion_equipos1`.`id`=`planificacion_pendientes`.`planificacion` LEFT JOIN `planificaciones` as planificaciones1 ON `planificaciones1`.`id`=`planificacion_equipos1`.`planificacion` LEFT JOIN `equipos` as equipos1 ON `equipos1`.`id`=`planificacion_equipos1`.`equipo` "
		);
		...
		$pkey = array(   
			...
			'tecnicos' => 'id',
			'planificaciones' => 'id',
			'planificacion_equipos' => 'id',
			'codigo_servicios' => 'id',
			'camionetas' => 'id',
			'clientes' => 'id',
			'planificacion_pendientes' => 'id'
		);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> servicio</a>
			</div>
